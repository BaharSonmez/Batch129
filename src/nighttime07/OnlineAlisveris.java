...
public class OnlineAlisveris {
    ...
    private static void musteriSecim(){
        System.out.println("Lutfen sectiginiz ururnun kodunu giriniz..");
        int secim = input.nextInt();
        System.out.println("Sectiginiz urunden kac adet alicaksiniz");
        int adet = input.nextInt();

        double urunTutari = adet*urunFiyatlari.get(secim-1);
        //get metodu index ile calisir secim-1 oldu.
        toplamOdeme+=urunTutari;
        System.out.println("Alisverise devam ise 1: \nOdeme icin 2: basin");
        int devamMi = input.nextInt();

        if(devamMi==1){
            musteriSecim();//recursive method (method icinden method cagir)
        }else
            odeme();
